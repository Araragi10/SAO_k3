...
func GenerateClients(settings interfaces.WSettings) {
	...
	client1, err := GenerateClient(RAW_CONFIG_CLIENT)
	if err != nil {
		log.Fatal(err)
	}
	...
	// pat client
	client2, err := GenerateClient(PAT_CLIENT)
	if err != nil {
		log.Fatal(err)
	}
...
func GenerateClient(index dbTypes.DB_INDEX) (interfaces.WClient, error) {
	if clientsMap == nil {
		clientsMap = make(map[dbTypes.DB_INDEX]interfaces.WClient)
		clientsSync = &sync.Mutex{}
	}

	if clientsMap[index] != nil {
		return clientsMap[index], nil
	}
	...
	_c._setClient()
	if index == RAW_CONFIG_CLIENT {
		re, err := _c.GetWotoConfiguration()
		if err != nil {
			return nil, err
		}

		if re != wa.SUCCESS {
			return nil, errors.New("unknown error in db")
		}
	}
...
func (_c *WotoClient) ResetWotoConfiguration() (result wa.RESULT, err error) {

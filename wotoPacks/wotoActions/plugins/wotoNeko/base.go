...
func GetRandomTGifD(searchQuery string) (base *TenorBase, err error) {
	list, err := GetTenorListD(searchQuery)
	if err != nil {
		return nil, err
	}

	return list.GetRandom(), nil
}

func GetTenorListD(qu string) (base *TenorList, err error) {
	req, err := http.NewRequest(wv.Get, getDreq(&qu), nil)
	if err != nil {
		return nil, err
	}

	req.Header.Set(userAgentKey, agentValue)
	req.Header.Set(wv.AcceptKey, wv.AcceptAll)
	req.Header.Set(acceptLanguageKey, wv.EnUs)
	req.Header.Set(authKey, genDAuth())
	req.Header.Set(xSuperKey, superKey)
	req.Header.Set(altUsedKey, dAlt)
	req.Header.Set(wv.ConnectionKey, wv.KeepAlive)
	req.Header.Set(wv.RefererKey, ghadiriChannel)

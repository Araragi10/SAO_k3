...
func TranslateD(fr, to, text string) *WotoTr {
	uText := wv.EMPTY
	for _, c := range text {
		uText += string(c)
	}
...
func parseGData(wTr *WotoTr) *WotoTr {
	...
	accepted := func(v string) bool {
		...
		tmp = strings.ReplaceAll(tmp, wv.LineEscape, wv.EMPTY)
...
func arrangeParams(values []string, wTr *WotoTr) {
	...
	for i, current := range values {
		...
		if strings.Contains(current, WrongNessOpen) {
			...
			if wTr.Road != nil {
				if !wTr.Road[index] {
					current += wv.LineEscape
				} else {
					current = strings.TrimPrefix(current, wv.LineEscape)
					current = strings.TrimSuffix(current, wv.LineEscape)
					current += wv.Point
				}
			}
...
func purify(text string) string {
	...
	if strings.Contains(text, wv.LineEscape) {
		text = strings.ReplaceAll(text, wv.LineEscape, wv.SPACE_VALUE)
	}

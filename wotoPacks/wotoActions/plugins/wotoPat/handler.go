...
func PatHandler(message *tg.Message, args *pTools.EventArgs) {
	...
	is_hentai := args.GetAsBool(HENTAI_FLAG)
	send_pv := args.GetAsBool(PRIVATE_FLAG, PV_FLAG)
	is_reply := message.ReplyToMessage != nil
	is_del := is_reply && args.GetAsBool(DEL_FLAG, DELETE_FLAG)
	only_photo := args.GetAsBool(PH_FLAG, PHOTO_FLAG)
...
func PatSHandler(message *tg.Message, args *pTools.EventArgs) {
	settings := appSettings.GetExisting()
	if settings == nil {
		return
	}
	...
	if is_add {
		if is_hentai {
			...
			newM, err := patCli.AddHPat(id, int32(PHOTO_PAT))
			if err != nil {
				log.Println(err)
				return
			}
			...
			newM, err := patCli.AddPat(id, int32(PHOTO_PAT))
			if err != nil {
				log.Println(err)
				return
			}

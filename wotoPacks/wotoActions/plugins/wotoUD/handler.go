...
import (
	...
	wm "github.com/Araragi10/SAO_k3/wotoPacks/wotoMD"
...
func UdHandler(message *tg.Message, args *pTools.EventArgs) {
	...
	del := reply && args.GetAsBool(DEL_FLAG, DELETE_FLAG)
	single := args.GetAsBool(SINGLE_FLAG)
	text := args.GetAsStringTOrRaw(TEXT_FLAG, TXT_FLAG)
	...
	if !found {
		...
		if !simOK {
			...
			for _, s := range sim {
				...
				mean += strconv.Itoa(num) + numSep + s + wv.LineEscape
				num++
			}
...
func QUdHanler(query *tg.CallbackQuery, q *wq.QueryBase) {
	...
	if udData.next {
		...
		for _, current := range vUrls {
			...
			tmpV = wv.LineEscape + iStr + numSep

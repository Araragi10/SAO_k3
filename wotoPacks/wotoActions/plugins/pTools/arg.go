...
func ParseArg(text string) (e *EventArgs, err error) {
	...
	if !ss.HasPrefix(wv.COMMAND_PREFIX1, wv.COMMAND_PREFIX2, wv.SUDO_PREFIX1) {
	...
	cmdSs := cmd.TrimStr(wv.COMMAND_PREFIX1, wv.COMMAND_PREFIX2,
		wv.SUDO_PREFIX1, wv.SPACE_VALUE)
	if cmdSs.IsEmpty() {

...
const (
	...
	FoxCmd       = "fox"       // wotoNeko plugin
	FoxyCmd      = "foxy"      // wotoNeko plugin
	gifdCmd      = "gifd"      // wotoNeko plugin
	mp4Cmd       = "mp4"       // wotoNeko plugin
	mediumgifCmd = "mediumgif" // wotoNeko plugin
	nanogifCmd   = "nanogif"   // wotoNeko plugin
	tinywebmCmd  = "tinywebm"  // wotoNeko plugin
	gifCmd       = "gif"       // wotoNeko plugin
	tinymp4Cmd   = "tinymp4"   // wotoNeko plugin
	nanowebmCmd  = "nanowebm"  // wotoNeko plugin
	nanomp4Cmd   = "nanomp4"   // wotoNeko plugin
	loopedmp4Cmd = "loopedmp4" // wotoNeko plugin
	webmCmd      = "webm"      // wotoNeko plugin
	tinygifCmd   = "tinygif"   // wotoNeko plugin
	udCmd        = "ud"        // wotoUD plugin
	defCmd       = "def"       // wotoUD plugin
	defineCmd    = "define"    // wotoUD plugin
	pingCmd      = "ping"      // wotoGP plugin
	banCmd       = "ban"       // wotoGP plugin
	infoCmd      = "info"      // wotoGP plugin
)

var cmdList map[string]CmdHandler
var cmdListMutex *sync.RWMutex
...
func addNekosLifeCmdList() {
	...
	if cmdList != nil {
		...
		if cmdList[gifdCmd] == nil {
			cmdList[gifdCmd] = gifDNekoHandler
		}

		if cmdList[mp4Cmd] == nil {
			cmdList[mp4Cmd] = mp4Handler
		}

		if cmdList[mediumgifCmd] == nil {
			cmdList[mediumgifCmd] = mediumgifHandler
		}

		if cmdList[nanogifCmd] == nil {
			cmdList[nanogifCmd] = nanogifHandler
		}

		if cmdList[tinywebmCmd] == nil {
			cmdList[tinywebmCmd] = tinywebmHandler
		}

		if cmdList[gifCmd] == nil {
			cmdList[gifCmd] = gifHandler
		}

		if cmdList[tinymp4Cmd] == nil {
			cmdList[tinymp4Cmd] = tinymp4Handler
		}

		if cmdList[nanowebmCmd] == nil {
			cmdList[nanowebmCmd] = nanowebmHandler
		}

		if cmdList[nanomp4Cmd] == nil {
			cmdList[nanomp4Cmd] = nanomp4Handler
		}

		if cmdList[loopedmp4Cmd] == nil {
			cmdList[loopedmp4Cmd] = loopedmp4Handler
		}

		if cmdList[webmCmd] == nil {
			cmdList[webmCmd] = webmHandler
		}

		if cmdList[tinygifCmd] == nil {
			cmdList[tinygifCmd] = tinygifHandler
		}
	}
}
...
func addGpCmdList() {
	if cmdList != nil {
		if cmdList[pingCmd] == nil {
			cmdList[pingCmd] = pingHandler
		}

		if cmdList[banCmd] == nil {
			cmdList[banCmd] = banHandler
		}

		if cmdList[infoCmd] == nil {
			cmdList[infoCmd] = infoHandler
		}
	}
}
